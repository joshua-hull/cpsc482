...
for STUDENT in $STUDENTS; do
	...
	if [[ $GOOD -eq '1' ]]; then
		echo \\nBuilding $STUDENT\\n
		xcodebuild -configuration Debug -sdk iphonesimulator8.1 | xcpretty
		...
		if [[ ${PIPESTATUS[0]} -ne '0' ]]; then
			...
			if [[ ${IOSSIMVERSION[0]} -lt '3' ]]; then
				...
				ios-sim launch *.app --exit --timeout 120 --devicetypeid "com.apple.CoreSimulator.SimDeviceType.iPhone-5, 8.1"
			fi
		fi
		echo \\n
	else
